...
{
    // Iniciem la vista que ens mostra el contingut del mail seleccionat.
...
- (void)cargarMails
{
    ...
    // Definim la url que ens mostra les dades dels missatges unread
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/messages/unread?access_token=%@",self.aula.identifier, self.tauler.identifier, self.auth.accessToken]];
    dispatch_queue_t backgroundQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
    
    dispatch_async(backgroundQueue, ^{
        NSData *messagesData = [NSData dataWithContentsOfURL:messagesURL];
        NSLog(@"Unread data - %@",[[NSString alloc] initWithData:messagesData encoding:NSUTF8StringEncoding]);
