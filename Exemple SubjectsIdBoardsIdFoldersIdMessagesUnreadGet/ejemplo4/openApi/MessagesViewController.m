...
{
    // Iniciem la vista que ens mostra el missatge seleccionat.
...
- (void)carregarMessages
{
    ...
    // Definim la URL que en donara les dades dels Missatges Unread.
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/folders/%@/messages/unread?access_token=%@",self.aula.identifier, self.board.identifier, self.folder.identifier , self.auth.accessToken]];
