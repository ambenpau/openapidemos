...
{
    // Iniciem la vista que ens mostra les carpetes dins el tauler seleccionat.
    FoldersViewController *foldersViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"foldersView"];
    foldersViewController.board = [self.boards objectAtIndex:indexPath.row];
...
- (void)cargarBoards
{
    ...
    NSURL *boardURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards?access_token=%@", self.aula.identifier, self.auth.accessToken]];
