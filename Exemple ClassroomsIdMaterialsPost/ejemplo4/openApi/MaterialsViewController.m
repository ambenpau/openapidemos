...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //resourceViewController.aula = self.aula;  Not needed because domainId of resource is the classroom id
...
- (void)cargarMaterials
{
    ...
    // Definim la URL que ens obte les dades dels materials
    NSURL *matURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/materials?access_token=%@", self.aula.identifier, self.auth.accessToken]];
    
    dispatch_queue_t backgroundQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
    
    dispatch_async(backgroundQueue, ^{
        NSData *matData = [NSData dataWithContentsOfURL:matURL ];
        NSLog(@"teachingMaterialList - %@", [[NSString alloc] initWithData:matData encoding:NSUTF8StringEncoding]);
...
{
    // Eliminem els materials que hi havia per a evitar duplicats.
...
-(IBAction)postMaterial
{
    // Creem un Diccionari amb els camps de material.
    NSDictionary *recursPost = @{@"id": @"545844", @"type":@"WEB", @"title":@"Google", @"url":@"www.google.com"};
    ...
    NSURL *resourcesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/materials?access_token=%@", self.aula.identifier, self.auth.accessToken]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:resourcesURL];
    
    // Indiquem que volem utilitzar el metode POST
