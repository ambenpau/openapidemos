//
//  DestinationFoldersViewController.m
...
@interface DestinationFoldersViewController ()

@end

@implementation DestinationFoldersViewController
...
{
    // Definim la carpeta desti com la carpeta seleccionada.
    Folder *folder = [self.folders objectAtIndex:indexPath.row];
    // Definim la URL que indica que movem el mail a la carpeta seleccionada.
    NSURL *resourcesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/mail/messages/%@/move/%@?access_token=%@", self.message.identifier , folder.identifier , self.auth.accessToken]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:resourcesURL];
    
    // Indiquem que el metode es POST.
...
- (void)cargarFolders
{
    ...
    // Definim la URL que ens donara les possibles carpetes desti.
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/mail/folders?access_token=%@",  self.auth.accessToken]];
