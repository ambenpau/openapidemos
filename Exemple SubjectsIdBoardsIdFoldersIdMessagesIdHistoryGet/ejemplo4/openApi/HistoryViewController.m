...
-(void) carregaMessage
{
    ...
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/folders/%@/messages/%@/history?access_token=%@",self.aula.identifier, self.board.identifier, self.folder.identifier , self.message.identifier , self.auth.accessToken]];
