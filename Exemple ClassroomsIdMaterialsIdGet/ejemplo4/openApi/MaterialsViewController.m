...
{
    // Iniciem la vista que carrega el material amb un identificador concret.
    MaterialViewController *materialViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"materialView"];
    materialViewController.idMat = [[self.materials objectAtIndex:indexPath.row] identifier];
    materialViewController.auth = self.auth;
    materialViewController.aula = self.aula;
...
- (void)cargarMaterials
{
    ...
    dispatch_async(backgroundQueue, ^{
        ...
        NSLog(@"Data mat - %@",[[NSString alloc]initWithData:matData encoding:NSUTF8StringEncoding]);
