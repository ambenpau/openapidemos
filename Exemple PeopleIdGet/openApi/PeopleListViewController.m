...
{
    // Iniciem la vista de la persona amb la persona seleccionada.
    PeopleViewController *peopleViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"peopleView"];
    peopleViewController.auth = self.auth;
    peopleViewController.idPeople = [[self.persons objectAtIndex:indexPath.row] identifier];
...
- (void)cargarPerson
{
    ...
    NSURL *peopleURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/people?access_token=%@", self.auth.accessToken]];
