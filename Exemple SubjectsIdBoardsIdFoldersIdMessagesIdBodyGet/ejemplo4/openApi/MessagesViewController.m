...
{
    // Iniciem la vista que ens mostrara el body del missatge seleccionat.
    BodyViewController *mesViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"bodyView"];
    mesViewController.message = [self.messages objectAtIndex:indexPath.row];
    mesViewController.auth = self.auth;
    mesViewController.aula = self.aula;
    mesViewController.board = self.board;
    mesViewController.folder = self.folder;
...
- (void)carregarMessages
{
    ...
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/folders/%@/messages?access_token=%@",self.aula.identifier, self.board.identifier, self.folder.identifier , self.auth.accessToken]];
