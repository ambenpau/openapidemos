...
{
    [super viewDidLoad];
    self.mail = [[MailHistory alloc]init];
    [self cargarMail];
}
...
- (void)cargarMail
{
    ...
    // Definimos la URL de la API que contiene los datos del history del mail.
    // Acuerdate de adjuntar en la query de la URI el token!
    NSURL *userURL = [NSURL URLWithString:
                      [NSString stringWithFormat:
                       @"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/folders/%@/messages/%@/history?access_token=%@", self.aula.identifier,self.board.identifier,self.folder.identifier, self.idMail ,self.auth.accessToken]];
    ...
    dispatch_async(backgroundQueue, ^{
        ...
        NSLog(@"History - %@",[[NSString alloc]initWithData:userData encoding:NSUTF8StringEncoding]);
...
{
    self.identifier.text    = [[NSString alloc] initWithFormat:@"id: %@", self.mail.identifier];
    self.details.text = self.mail.details;
}
