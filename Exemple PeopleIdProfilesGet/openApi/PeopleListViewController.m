...
{
    // Iniciem la vista que mostra els perfils de la persona seleccionada.
    ProfilesViewController *pfViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"profilesView"];
    pfViewController.auth = self.auth;
    pfViewController.person = [self.persons objectAtIndex:indexPath.row];
    [self.navigationController pushViewController:pfViewController animated:YES];
}
