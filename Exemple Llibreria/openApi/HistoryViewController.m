...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MessageHistoryDetail *messageH = [self.history objectAtIndex:indexPath.row];
    
    // Recuperamos el nombre que queremos poner de titulo a la celda
    NSString *CellIdentifier = [[NSString alloc] initWithString:messageH.name];
...
- (void)cargarMessages
{
    ...
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/mail/messages/%@/history?access_token=%@", self.idMessage,self.auth.accessToken]];
    ...
    dispatch_async(backgroundQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.history addObjectsFromArray:self.messagehistory.details];
