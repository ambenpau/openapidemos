...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Definim la URL que fara l'accio de moure el missatge a la carpeta seleccionada.
    NSURL *resourcesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/messages/%@/move/%@?access_token=%@", self.aula.identifier, self.board.identifier , self.message.identifier , folder.identifier , self.auth.accessToken]];
...
- (void)cargarFolders
{
    ...
    // Definim la URL que ens donara les dades de les possibles carpetes desti.
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/folders?access_token=%@", self.aula.identifier, self.board.identifier, self.auth.accessToken]];
