...
-(void) carregaMessage
{
    ...
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/folders/%@/messages/%@?access_token=%@",self.aula.identifier, self.board.identifier, self.folder.identifier , self.idMes , self.auth.accessToken]];
