...
#import "FoldersMoveViewController.h"

@interface FoldersMoveViewController ()

@end

@implementation FoldersMoveViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Folder *folder = [self.folders objectAtIndex:indexPath.row];
    // Definim la url amb la qual farem el moviment del missatge segons la carpeta desti seleccionada
    NSURL *resourcesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/folders/%@/messages/%@/move/%@?access_token=%@", self.aula.identifier, self.board.identifier ,self.oriFolder.identifier , self.mail.identifier , folder.identifier , self.auth.accessToken]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:resourcesURL];
    
    // Indiquem que el metode sigui POST
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[[NSOperationQueue alloc] init]completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        if ([data length] >0 && error == nil)
        {
            ...
            // Com que la vista nomes es pot modificar en la cua principal, si volem crear un alertView hem de iniciar una cua.
...
- (void)cargarFolders
{
    ...
    // Definim la URL per a mostrar les carpetes desti
