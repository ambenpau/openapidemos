...
- (void)cargarMail
{
    ...
    NSURL *userURL = [NSURL URLWithString:
                      [NSString stringWithFormat:
                       @"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/folders/%@/messages/%@?access_token=%@", self.aula.identifier,self.board.identifier,self.folder.identifier, self.idMail ,self.auth.accessToken]];
...
{
    FoldersMoveViewController *fmViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"fmView"];
    fmViewController.auth = self.auth;
    fmViewController.board = self.board;
    fmViewController.aula = self.aula;
    fmViewController.mail = self.mail;
    fmViewController.oriFolder = self.folder;
