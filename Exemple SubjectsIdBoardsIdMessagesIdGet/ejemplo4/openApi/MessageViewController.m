...
-(void) carregarMessage
{
    [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:YES];
    
    // Definim la URL que ens donara les dades del missatge amb identificador idMes
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects/%@/boards/%@/messages/%@?access_token=%@",self.aula.identifier, self.board.identifier, self.idMes, self.auth.accessToken]];
    ...
    dispatch_async(backgroundQueue, ^{
        ...
        NSLog(@"messageData - %@", [[NSString alloc] initWithData:messagesData encoding:NSUTF8StringEncoding]);
