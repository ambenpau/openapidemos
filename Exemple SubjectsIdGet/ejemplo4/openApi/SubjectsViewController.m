...
{
    // Iniciem la vista que ens carregara i mostara l'aula seleccionada.
    SubjectViewController *subjectViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"subjectView"];
    subjectViewController.idAula = [[self.classrooms objectAtIndex:indexPath.row] identifier];
    subjectViewController.auth = self.auth;
...
- (void)cargarAulas
{
    ...
    NSURL *classroomsURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/subjects?access_token=%@", self.auth.accessToken]];
