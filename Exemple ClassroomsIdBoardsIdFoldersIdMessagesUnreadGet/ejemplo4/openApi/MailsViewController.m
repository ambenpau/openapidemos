...
{
    //Iniciem la vista que mostra el mail seleccionat.
...
- (void)cargarMails
{
    ...
    //Iniciem la URL per a obtenir els missatges de unread.
    NSURL *messagesURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/folders/%@/messages/unread?access_token=%@",self.aula.identifier, self.tauler.identifier, self.folder.identifier ,self.auth.accessToken]];
    dispatch_queue_t backgroundQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
    
    dispatch_async(backgroundQueue, ^{
        NSData *messagesData = [NSData dataWithContentsOfURL:messagesURL];
        NSLog(@"unread - %@",[[NSString alloc]initWithData:messagesData encoding:NSUTF8StringEncoding]);
