...
- (void)cargarMail
{
    ...
    // Definimos la URL de la API que contiene los datos del body del mail.
    // Acuerdate de adjuntar en la query de la URI el token!
    NSURL *userURL = [NSURL URLWithString:
                      [NSString stringWithFormat:
                       @"http://denver.uoc.es:8080/webapps/uocapi/api/v1/classrooms/%@/boards/%@/folders/%@/messages/%@/body?access_token=%@", self.aula.identifier,self.board.identifier,self.folder.identifier, self.idMail ,self.auth.accessToken]];
